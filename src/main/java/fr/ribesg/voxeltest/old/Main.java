    ...
    public static void main(final String[] args) {
        ...
        for (int i = 0; i < Main.voxels.length; i++) {
            for (int j = 0; j < Main.voxels.length; j++) {
                //Main.voxels[i][j] = (byte) ((i - X.CHUNK_SIZE / 2) * (i - X.CHUNK_SIZE / 2) + (j - X.CHUNK_SIZE / 2) * (j - X.CHUNK_SIZE / 2) - (X.CHUNK_SIZE / 2 * X.CHUNK_SIZE / 2));
                //Main.voxels[i][j] = (byte) X.RANDOM.nextInt();
                Main.voxels[i][j] = i % 16 >= 8 ? (j % 16 >= 8 ? Byte.MIN_VALUE : Byte.MAX_VALUE) : (j % 16 < 8 ? Byte.MIN_VALUE : Byte.MAX_VALUE);
